...
function h($ig)
{
    return str_replace(
        "\0",
        "&#0;",
        htmlspecialchars($ig, ENT_QUOTES, "utf-8")
    );
}
function nl_br($ig)
{
...
function checkbox($E, $Y, $Na, $qd = "", $qe = "", $Ra = "", $rd = "")
{
    $K =
        ...
        "'" .
        ($Na ? " checked" : "") .
        ($rd ? " aria-labelledby='$rd'" : "") .
        ">" .
        ($qe ? script("qsl('input').onclick = function () { $qe };", "") : "");
    return $qd != "" || $Ra
        ? "<label" . ($Ra ? " class='$Ra'" : "") . ">$K" . h($qd) . "</label>"
        : $K;
}
function optionlist($ue, $Kf = null, $lh = false)
{
    ...
    foreach ($ue as $md => $W) {
        ...
        foreach ($ve as $z => $X) {
            $K .=
                "<option" .
                ($lh || is_string($z) ? ' value="' . h($z) . '"' : "") .
                (($lh || is_string($z) ? (string) $z : $X) === $Kf
                    ? " selected"
                    : "") .
                ">" .
                h($X);
...
function html_select($E, $ue, $Y = "", $pe = true, $rd = "")
{
    if ($pe) {
        return "<select name='" .
            ...
            "</select>" .
            (is_string($pe)
                ? script("qsl('select').onchange = function () { $pe };", "")
                : "");
    }
    $K = "";
    foreach ($ue as $z => $X) {
...
function select_input($wa, $ue, $Y = "", $pe = "", $Ue = "")
{
    ...
    return "<$xg$wa" .
        ($ue
            ? "><option value=''>$Ue" . optionlist($ue, $Y, true) . "</select>"
            : " size='10' value='" . h($Y) . "' placeholder='$Ue'>") .
        ($pe ? script("qsl('$xg').onchange = $pe;", "") : "");
}
function confirm($D = "", $Lf = "qsl('input')")
{
    return script(
        "$Lf.onclick = function () { return confirm('" .
            ($D ? js_escape($D) : "Sind Sie sicher?") .
            "'); };",
        ""
    );
}
function print_fieldset($u, $yd, $uh = false)
{
    echo "<fieldset><legend>",
        "<a href='#fieldset-$u'>$yd</a>",
        script("qsl('a').onclick = partial(toggle, 'fieldset-$u');", ""),
        "</legend>",
        "<div id='fieldset-$u'" . ($uh ? "" : " class='hidden'") . ">\n";
}
function bold($Ga, $Ra = "")
{
        ...
        $K[] =
            ...
            ($y == "sql" && is_numeric($X) && preg_match("~\.~", $X)
                ...
                : ($y == "mssql"
                    ...
                    : " = " . unconvert_field($m[$z], q($X))));
        if (
            $y == "sql" &&
            preg_match("~char|text~", $m[$z]["type"]) &&
            preg_match("~[^ -@]~", $X)
        ) {
...
function cookie($E, $Y, $Ad = 2592000)
{
    ...
    return header(
        "Set-Cookie: $E=" .
            urlencode($Y) .
            ($Ad
                ? "; expires=" . gmdate("D, d M Y H:i:s", time() + $Ad) . " GMT"
                : "") .
            "; path=" .
...
function get_file($z, $yb = false)
{
    ...
    foreach ($uc["error"] as $z => $k) {
        ...
        $gb = file_get_contents(
            $yb && preg_match('~\.gz$~', $E) ? "compress.zlib://$Lg" : $Lg
        );
        if ($yb) {
            $eg = substr($gb, 0, 3);
            if (
                function_exists("iconv") &&
                preg_match("~^\xFE\xFF|^\xFF\xFE~", $eg, $vf)
            ) {
...
function upload_error($k)
{
    ...
    return $k
        ? "Hochladen von Datei fehlgeschlagen." .
                ($Ld
                    ? " " . sprintf("Maximal erlaubte Dateigröße ist %sB.", $Ld)
                    : "")
        : "Datei existiert nicht.";
}
function repeat_pattern($Re, $zd)
{
    return str_repeat("$Re{0,65535}", $zd / 65535) .
        "$Re{0," .
        $zd % 65535 .
        "}";
}
...
function input($l, $Y, $q)
{
    ...
    $Fc =
        (isset($_GET["select"]) || $xf ? ["orig" => "Original"] : []) +
        $b->editFunctions($l);
    $wa = " name='fields[$E]'";
    if ($l["type"] == "enum") {
        ...
        if ($dd != "") {
            ...
            ($Bg = preg_match("~text|lob|memo~i", $l["type"])) ||
            preg_match("~\n~", $Y)
        ) {
            if ($Bg && $y != "sqlite") {
                $wa .= " cols='50' rows='12'";
            ...
            $Nd =
                ...
                preg_match('~^(\d+)(,(\d+))?$~', $l["length"], $C)
                    ? (preg_match("~binary~", $l["type"]) ? 2 : 1) * $C[1] +
                        ($C[3] ? 1 : 0) +
                        ($C[2] && !$l["unsigned"] ? 1 : 0)
                    : ($Xg[$l["type"]]
                        ? $Xg[$l["type"]] + ($l["unsigned"] ? 0 : 1)
                        : 0);
            if (
                $y == "sql" &&
                min_version(5.6) &&
            ...
            echo "<input" .
                ...
                h($Y) .
                "'" .
                ($Nd ? " data-maxlength='$Nd'" : "") .
                (preg_match("~char|binary~", $l["type"]) && $Nd > 20
                    ? " size='40'"
                    : "") .
                "$wa>";
        }
        ...
        if (
            ...
            if (!$J || $J->fetch_row()) {
                $df =
                    ...
                    "'>$E</a>";
                echo "$Nf<li>" .
                    ($J ? $df : "<p class='error'>$df: " . error()) .
                    "\n";
                $Nf = "";
            }
        }
    }
    echo ($Nf ? "<p class='message'>" . "Keine Tabellen." : "</ul>") . "\n";
}
...
function apply_sql_function($q, $c)
{
    return $q
        ? ($q == "unixepoch"
            ? "DATETIME($c, '$q')"
            : ($q == "count distinct" ? "COUNT(DISTINCT " : strtoupper("$q(")) .
                "$c)")
        : $c;
}
...
function select_value($X, $A, $l, $Cg)
{
    ...
    if (is_array($X)) {
        ...
        foreach ($X as $md => $W) {
            $K .=
                "<tr>" .
                ($X != array_values($X) ? "<th>" . h($md) : "") .
                "<td>" .
                select_value($W, $A, $l, $Cg);
        }
...
function edit_form($Q, $m, $L, $fh)
{
    ...
    if ($L === false) {
        echo "<p class='error'>" . "Keine Datensätze." . "\n";
    }
    ...
    if (!$m) {
        ...
        foreach ($m as $E => $l) {
            ...
            $q = $_POST["save"]
                ...
                : ($fh && preg_match("~^CURRENT_TIMESTAMP~i", $l["on_update"])
                    ? "now"
                    : ($Y === false
                        ? null
                        : ($Y !== null
...
class Min_SQL
{
    ...
    function select($Q, $N, $Z, $s, $we = [], $_ = 1, $F = 0, $df = false)
    {
        global $b, $y;
        $jd = count($s) < count($N);
...
class Adminer
{
    ...
    function selectLinks($qg, $P = "")
    {
        global $y, $j;
        echo '<p class="links">';
        $Bd = ["select" => "Daten auswählen"];
        if (support("table") || support("indexes")) {
            $Bd["table"] = "Struktur anzeigen";
        }
        if (support("table")) {
            if (is_view($qg)) {
                $Bd["view"] = "View ändern";
            } else {
                $Bd["create"] = "Tabelle ändern";
            }
        }
        if ($P !== null) {
            $Bd["edit"] = "Neuer Datensatz";
        }
        $E = $qg["Name"];
        foreach ($Bd as $z => $X) {
            echo " <a href='" .
    ...
    function tableStructurePrint($m)
    {
        echo "<div class='scrollable'>\n",
            ...
            "<thead><tr><th>" .
                ...
                "<td>" .
                "Typ" .
                (support("comment") ? "<td>" . "Kommentar" : "") .
                "</thead>\n";
        foreach ($m as $l) {
            echo "<tr" . odd() . "><th>" . h($l["field"]),
                "<td><span title='" .
                    h($l["collation"]) .
                    "'>" .
                    h($l["full_type"]) .
                    "</span>",
                $l["null"] ? " <i>NULL</i>" : "",
                $l["auto_increment"] ? " <i>" . "Auto-Inkrement" . "</i>" : "",
                isset($l["default"])
                    ? " <span title='" .
    ...
    {
        global $Fc, $Jc;
        print_fieldset("select", "Daten zeigen von", $N);
        $t = 0;
        $N[""] = [];
        foreach ($N as $z => $X) {
            $X = $_GET["columns"][$z];
    ...
    {
        print_fieldset("search", "Suchen", $Z);
        foreach ($w as $t => $v) {
            if ($v["type"] == "FULLTEXT") {
                echo "<div>(<i>" .
    ...
    function selectOrderPrint($we, $d, $w)
    {
        ...
        select_input(" name='order[$t]'", $d, "", "selectAddRow"),
            checkbox("desc[$t]", 1, false, "absteigend") . "</div>\n",
            "</div></fieldset>\n";
    }
    function selectLimitPrint($_)
    {
        echo "<fieldset><legend>" . "Begrenzung" . "</legend><div>";
    ...
    {
        if ($Cg !== null) {
            echo "<fieldset><legend>" . "Textlänge" . "</legend><div>",
    ...
    {
        echo "<fieldset><legend>" . "Aktion" . "</legend><div>",
            "<input type='submit' value='" . "Daten zeigen von" . "'>",
        ...
        foreach ((array) $_GET["where"] as $z => $X) {
            if (
                ...
                if (preg_match('~IN$~', $X["op"])) {
                    ...
                    $db =
                        " LIKE " .
                        $this->processInput($m[$X["col"]], "%$X[val]%");
                } elseif ($X["op"] == "ILIKE %%") {
                    $db =
                        " ILIKE " .
                        $this->processInput($m[$X["col"]], "%$X[val]%");
                ...
                if ($X["col"] != "") {
                    ...
                    foreach ($m as $E => $l) {
                        if (
                            ...
                            $Ya[] =
                                $af .
                                $j->convertSearch(idf_escape($E), $X, $l) .
                                $db;
                        }
                    }
    ...
    function messageQuery($I, $Dg, $qc = false)
    {
        ...
        $K = "<a href='#$cg' class='toggle'>" . "SQL-Kommando" . "</a>\n";
        if (!$qc && ($wh = $j->warnings())) {
        ...
        return " <span class='time'>" .
            @date("H:i:s") .
            "</span>" .
            " $K<div id='$cg' class='hidden'><pre><code class='jush-$y'>" .
            shorten_utf8($I, 1000) .
            "</code></pre>" .
            ($Dg ? " <span class='time'>($Dg)</span>" : "") .
            (support("sql")
                ? '<p><a href="' .
                    h(
                        str_replace(
                    ...
                    if ($_POST["format"] != "sql") {
                        ...
                        if (!$ed) {
                            $ed =
                                ...
                                table($Q) .
                                " (" .
                                implode(", ", array_map("idf_escape", $nd)) .
                                ") VALUES";
                        }
                        foreach ($L as $z => $X) {
                            $l = $m[$z];
                        ...
                        if (!$Ia) {
                            ...
                            strlen($Ia) + 4 + strlen($Ff) + strlen($mg) <
                            $Id
                        ) {
                            $Ia .= ",$Ff";
                        } else {
        ...
        ($_GET["ns"] == "" && support("database")
            ...
            support("scheme")
                ? "<a href='" .
                    h(ME) .
                    "scheme='>" .
                    ($_GET["ns"] != "" ? "Schema ändern" : "Schema erstellen") .
                    "</a>\n"
                : "",
            $_GET["ns"] !== ""
                ? '<a href="' .
                ...
                : "",
            support("privileges")
                ? "<a href='" . h(ME) . "privileges='>" . "Rechte" . "</a>\n"
                : "";
        return true;
    }
...
<a href="https://www.adminer.org/#download"',
            target_blank(),
            ' id="version">',
            version_compare($ga, $_COOKIE["adminer_version"]) < 0
...
';
            ...
            if ($_GET["ns"] !== "" && !$Td && DB != "") {
                echo '<a href="' .
                    ...
                    "</a>\n";
                if (!$S) {
                    echo "<p class='message'>" . "Keine Tabellen." . "\n";
                } else {
                    $this->tablesPrint($S);
                }
            }
...
if (!defined("DRIVER")) {
    ...
    class Min_Driver extends Min_SQL
    {
        ...
        function convertSearch($Wc, $X, $l)
        {
            return preg_match("~char|text|enum|set~", $l["type"]) &&
                ...
                preg_match('~[\x80-\xFF]~', $X["val"])
                ? "CONVERT($Wc USING " . charset($this->_conn) . ")"
                : $Wc;
        }
        function warnings()
        {
    ...
    function connect()
    {
        ...
        if (
            function_exists("iconv") &&
            !is_utf8($K) &&
            strlen($Ff = iconv("windows-1250", "utf-8", $K)) > strlen($K)
        ) {
            $K = $Ff;
        }
        return $K;
    ...
    {
        return " $I$Z" .
            ($_ !== null ? $Of . "LIMIT $_" . ($he ? " OFFSET $he" : "") : "");
    }
    ...
    {
        return (min_version(5) && $i == "information_schema") ||
        ...
        if (!$wf || queries("RENAME TABLE " . implode(", ", $wf))) {
            ...
            foreach ($Bb as $E => $sh) {
                if (
                    !queries(
                        "CREATE VIEW $E AS " .
                            str_replace(" $i.", " ", $sh["select"])
                    ) ||
                    !queries("DROP VIEW $i.$E")
                ) {
                    return false;
                }
            }
    ...
    function copy_tables($S, $th, $yg)
    {
        ...
        foreach ($th as $Q) {
            ...
            if (
                ...
                !queries("CREATE VIEW $E AS $sh[select]")
            ) {
                return false;
            }
        }
    ...
    function routine($E, $U)
    {
        ...
        $Wg =
            "((" .
            implode("|", array_merge(array_keys($Xg), $qa)) .
...
mixin(document.body, {onkeydown: bodyKeydown, onclick: bodyClick',
        isset($_COOKIE["adminer_version"])
            ? ""
            : ", onload: partial(verifyVersion, '$ga', '" .
                js_escape(ME) .
                "', '" .
                get_token() .
                "')";
...
echo js_escape("Sie sind offline."),
    '\';
...
{
    $W = array_values(
        unpack("V*", str_pad($Ff, 4 * ceil(strlen($Ff) / 4), "\0"))
    );
    if ($vh) {
        $W[] = strlen($Ff);
    }
...
function auth_error($k)
{
    ...
    if (isset($_GET["username"])) {
        ...
        if (($_COOKIE[$Rf] || $_GET[$Rf]) && !$Oc) {
            $k = "Sitzungsdauer abgelaufen, bitte erneut anmelden.";
        } else {
            restart_session();
            add_invalid_login();
            $G = get_password();
    ...
    if (!$_COOKIE[$Rf] && $_GET[$Rf] && ini_bool("session.use_only_cookies")) {
        $k = "Unterstüzung für PHP-Sessions muss aktiviert sein.";
    }
...
if (
    !is_object($e) ||
    ($Cd = $b->login($_GET["username"], get_password())) !== true
) {
    $k = is_string($e)
        ? h($e)
        : (is_string($Cd)
...
function select($J, $f = null, $ze = [], $_ = 0)
{
    ...
    echo ($t
        ? "</table>\n</div>"
        : "<p class='message'>" . "Keine Datensätze.") . "\n";
    return $K;
}
...
function set_adminer_settings($Tf)
{
    return cookie(
        "adminer_settings",
        http_build_query($Tf + adminer_settings())
    );
}
...
function edit_type($z, $l, $Xa, $o = [], $pc = [])
{
    ...
    echo optionlist(array_merge($pc, $jg), $U),
    ...
    echo ' aria-labelledby="label-length"><td class="options">',
        ...
        $o
            ? "<select name='" .
                ...
                "ON DELETE" .
                ")" .
                optionlist(explode("|", $oe), $l["on_delete"]) .
                "</select> "
            : " ";
}
...
function process_type($l, $Va = "COLLATE")
{
    ...
    return " $l[type]" .
        ...
        (preg_match(number_type(), $l["type"]) && in_array($l["unsigned"], $eh)
            ? " $l[unsigned]"
            : "") .
...
function default_value($l)
{
    ...
    return $zb === null
        ? ""
        : " DEFAULT " .
                (preg_match("~char|binary|text|enum|set~", $l["type"]) ||
                preg_match("~^(?![a-z])~i", $zb)
                    ? q($zb)
                    : $zb);
}
...
';
    ...
    echo '<th id="label-name">',
        $U == "TABLE" ? "Spaltenname" : "Name des Parameters",
        '<td id="label-type">Typ<textarea id="enum-edit" rows="4" cols="12" wrap="off" style="display: none;"></textarea>',
        script("qs('#enum-edit').onblur = editingLengthBlur;"),
        '<td id="label-length">Länge
<td>',
        "Optionen";
    if ($U == "TABLE") {
        echo '<td id="label-null">NULL
<td><input type="radio" name="auto_increment_col" value=""><acronym id="label-ai" title="Auto-Inkrement">AI</acronym>',
            doc_link([
...
',
            support("comment")
                ? "<td id='label-comment'$cb>" . "Kommentar"
                : "";
    }
    echo "<td>",
        "<input type='image' class='icon' name='add[" .
            (support("move_col") ? 0 : count($m)) .
            "]' src='" .
            h(
...
',
    ...
    foreach ($m as $t => $l) {
        ...
        $Hb =
            (isset($_POST["add"][$t - 1]) ||
                (isset($l["field"]) && !$_POST["drop_col"][$t])) &&
            (support("drop_col") || $_e == "");
        echo "<tr",
...
function grant($r, $gf, $d, $ne)
{
    ...
    if ($gf == ["ALL PRIVILEGES", "GRANT OPTION"]) {
        return $r == "GRANT"
            ? queries("$r ALL PRIVILEGES$ne WITH GRANT OPTION")
            : queries("$r ALL PRIVILEGES$ne") && queries("$r GRANT OPTION$ne");
    }
    return queries(
        "$r " .
            preg_replace(
...
function drop_create($Lb, $g, $Mb, $Ag, $Nb, $B, $Qd, $Od, $Pd, $ke, $Zd)
{
    if ($_POST["drop"]) {
        ...
        queries_redirect(
            $B,
            $Od,
            queries($Ag) && queries($Nb) && queries($Lb) && queries($g)
        );
    }
}
...
function format_foreign_key($n)
{
    ...
    $de = $n["ns"];
    return " FOREIGN KEY (" .
        implode(", ", array_map("idf_escape", $n["source"])) .
        ") REFERENCES " .
        ($i != "" && $i != $_GET["db"] ? idf_escape($i) . "." : "") .
        ($de != "" && $de != $_GET["ns"] ? idf_escape($de) . "." : "") .
        table($n["table"]) .
        " (" .
        implode(", ", array_map("idf_escape", $n["target"])) .
        ")" .
        (preg_match("~^($oe)\$~", $n["on_delete"])
            ? " ON DELETE $n[on_delete]"
            : "") .
        (preg_match("~^($oe)\$~", $n["on_update"])
...
function doc_link($Qe, $Bg = "<sup>?</sup>")
{
    ...
    $ih = [
        ...
        "oracle" =>
            ...
            preg_replace("~^.* (\d+)\.(\d+)\.\d+\.\d+\.\d+.*~s", '\1\2', $Pf) .
            "&id=",
    ];
    if (preg_match("~MariaDB~", $Pf)) {
        $ih["sql"] = "https://mariadb.com/kb/en/library/";
        $Qe["sql"] = isset($Qe["mariadb"])
            ? $Qe["mariadb"]
            : str_replace(".html", "/", $Qe["sql"]);
    }
    return $Qe[$y]
        ? "<a href='" . h($ih[$y] . $Qe[$y]) . "'" . target_blank() . ">$Bg</a>"
        : "";
}
...
function connect_error()
{
    ...
    if (DB != "") {
        ...
        page_header("Datenbank" . ": " . h(DB), "Datenbank ungültig.", true);
    } else {
        if ($_POST["db"] && !$k) {
            queries_redirect(
                substr(ME, 0, -1),
        ...
        page_header("Datenbank auswählen", $k, false);
        echo "<p class='links'>\n";
        foreach (
            [
            ...
            foreach ($h as $i => $S) {
                ...
                (support("database")
                    ? "<a href='$Bf" .
                        ...
                        "'>$Wa</a>"
                    : $Wa),
                    "<td align='right'><a href='$Bf&amp;schema=' id='tables-" .
                        h($i) .
                        "' title='" .
                        "Datenbankschema" .
                        "'>" .
...
if (isset($_GET["download"])) {
    ...
    $bb = $R["Comment"];
    if ($bb != "") {
        echo "<p class='nowrap'>" . "Kommentar" . ": " . h($bb) . "\n";
    }
    if ($m) {
    ...
    if (!is_view($R)) {
        ...
        if (fk_support($R)) {
            echo "<h3 id='foreign-keys'>" . "Fremdschlüssel" . "</h3>\n";
            $o = foreign_keys($a);
            if ($o) {
                echo "<table cellspacing='0'>\n",
                    "<thead><tr><th>" .
                ...
                foreach ($o as $E => $n) {
                    echo "<tr title='" . h($E) . "'>",
                        "<th><i>" .
                            implode("</i>, <i>", array_map("h", $n["source"])) .
                            "</i>",
                        "<td><a href='" .
                            h(
                                $n["db"] != ""
                            ...
                            "</a>",
                        "(<i>" .
                            implode("</i>, <i>", array_map("h", $n["target"])) .
                            "</i>)",
                        "<td>" . h($n["on_delete"]) . "\n",
                        "<td>" . h($n["on_update"]) . "\n",
                        '<td><a href="' .
    ...
    if (support(is_view($R) ? "view_trigger" : "trigger")) {
        echo "<h3 id='triggers'>" . "Trigger" . "</h3>\n";
        $Ug = triggers($a);
        if ($Ug) {
            echo "<table cellspacing='0'>\n";
            foreach ($Ug as $z => $X) {
    ...
    foreach (table_status("", true) as $Q => $R) {
        ...
        foreach ($b->foreignKeys($Q) as $X) {
            if (!$X["db"]) {
                ...
                $Hf[$Q]["references"][$X["table"]][(string) $vd] = [
                    $X["source"],
                    $X["target"],
                ];
        ...
        foreach ((array) $Q["references"] as $zg => $uf) {
            foreach ($uf as $vd => $qf) {
                ...
                foreach ($qf[0] as $z => $Yf) {
                    ...
                    $Ye =
                        $Hf[$zg]["pos"][0] +
                        $Hf[$zg]["fields"][$qf[1][$z]]["pos"];
                    $Sd = min($Sd, $Xe, $Ye);
                    $Kd = max($Kd, $Xe, $Ye);
                }
                echo "<div class='references' id='refl$vd' style='left: $vd" .
                    "em; top: $Sd" .
...
<p class="links"><a href="',
        ...
        '" id="schema-link">Dauerhafter Link</a>
';
...
';
    ...
    echo "<tr><th>" .
        "Ergebnis" .
        "<td>" .
        html_select("output", $b->dumpOutput(), $L["output"], 0) .
        "\n";
    echo "<tr><th>" .
        "Format" .
        "<td>" .
        html_select("format", $b->dumpFormat(), $L["format"], 0) .
        "\n";
    echo $y == "sqlite"
    ? ""
    : "<tr><th>" .
        ...
        (support("routine")
            ? checkbox("routines", 1, $L["routines"], "Routinen")
            : "") .
        (support("event")
            ? checkbox("events", 1, $L["events"], "Ereignisse")
            : ""),
        "<tr><th>" .
            "Tabellen" .
            "<td>" .
            html_select("table_style", $ug, $L["table_style"]) .
            checkbox(
                "auto_increment",
                1,
                $L["auto_increment"],
            ...
            (support("trigger")
                ? checkbox("triggers", 1, $L["triggers"], "Trigger")
                : ""),
        "<tr><th>" .
            "Daten" .
            "<td>" .
    ...
    if (DB != "") {
        ...
        foreach ($vg as $E => $U) {
            ...
            if ($U !== null && !preg_match("~table~i", $U)) {
                ...
                echo "$df<td align='right'><label class='block'><span id='Rows-" .
                    h($E) .
                    "'></span>" .
                    checkbox("data[]", $E, $Na) .
                    "</label>\n";
...
';
    ...
    page_header(isset($_GET["import"]) ? "Importieren" : "SQL-Kommando", $k);
    if (!$k && $_POST) {
        $p = false;
        if (!isset($_GET["import"])) {
            $I = $_POST["query"];
            ...
            $p = @fopen(
                file_exists($bg) ? $bg : "compress.zlib://$bg.gz",
                "rb"
            );
        ...
        if (is_string($I)) {
            ...
            while ($I != "") {
                if (
                    !$he &&
                    preg_match("~^$Zf*+DELIMITER\\s+(\\S+)~i", $I, $C)
                ) {
                    $Cb = $C[1];
            ...
            if ($Yb) {
                echo "<p class='message'>" . "Kein Kommando vorhanden." . "\n";
                ...
                lang(
                    ...
                    " <span class='time'>(" . format_time($Ng) . ")</span>\n";
            } elseif ($dc && $ab > 1) {
                echo "<p class='error'>" .
                    "Fehler in der SQL-Abfrage" .
                    ": " .
...
';
    ...
    if (!isset($_GET["import"])) {
        ...
        echo "<fieldset><legend>" . "Datei importieren" . "</legend><div>";
        ...
        if ($Yc) {
            echo "<fieldset><legend>" . "Vom Server" . "</legend><div>",
                sprintf("Webserver Datei %s", "<code>" . h($Yc) . "$Kc</code>"),
...
';
    ...
    foreach ($m as $E => $l) {
        if (
            !isset($l["privileges"][$fh ? "update" : "insert"]) ||
            $b->fieldName($l) == "" ||
            $l["generated"]
        ) {
            unset($m[$E]);
    ...
    if ($_POST && !$k && !isset($_GET["select"])) {
        ...
        if (isset($_POST["delete"])) {
            ...
            if ($fh) {
                ...
                queries_redirect(
                    $B,
                    sprintf("Datensatz%s wurde eingefügt.", $ud ? " $ud" : ""),
                    $J
                );
            }
        }
    ...
    page_header(
        $a != "" ? "Tabelle ändern" : "Tabelle erstellen",
        $k,
        ["table" => $a],
        h($a)
    );
...
';
    if (support("columns") || $a == "") {
        echo 'Name der Tabelle: <input name="name" data-maxlength="64" value="',
            h($L["name"]),
            '" autocapitalize="off">
';
        if ($a == "" && !$_POST) {
...
',
            ...
            '</div>
<p>
Auto-Inkrement: <input type="number" name="Auto_increment" size="6" value="',
            h($L["Auto_increment"]),
            '">
',
            checkbox(
...
';
    if ($a != "") {
        echo '<input type="submit" name="drop" value="Entfernen">',
            confirm(sprintf("Drop %s?", $a));
    }
    if (support("partitioning")) {
        $Me = preg_match("~RANGE|LIST~", $L["partition_by"]);
        print_fieldset("partition", "Partitionieren um", $L["partition_by"]);
        echo '<p>
',
            "<select name='partition_by'>" .
                optionlist(["" => ""] + $Le, $L["partition_by"]) .
                "</select>" .
                on_help(
            ...
            h($L["partition"]),
            '">)
Partitionen: <input type="number" name="partitions" class="size',
            $Me || !$L["partition_by"] ? " hidden" : "",
            '" value="',
            h($L["partitions"]),
            '">
...
';
            ...
            if (in_array($v["type"], $ad)) {
                ...
                if ($d) {
                    ...
                    if ($kc) {
                        ...
                        if (
                            $v["type"] == $kc["type"] &&
                            array_values($kc["columns"]) === $d &&
                            (!$kc["lengths"] ||
                                array_values($kc["lengths"]) === $_d) &&
    ...
    page_header("Indizes", $k, ["table" => $a], h($a));
    ...
    foreach ($L["indexes"] as $v) {
        if (!$_POST["drop_col"] || $x != key($_POST["drop_col"])) {
            ...
            foreach ($v["columns"] as $z => $c) {
                echo "<span>" .
                select_input(
                    " name='indexes[$x][columns][$t]' title='" . "Spalte" . "'",
                    ...
                    $y == "sql" || $y == "mssql"
                        ? "<input type='number' name='indexes[$x][lengths][$t]' class='size' value='" .
                            h($v["lengths"][$z]) .
                            "' title='" .
                            "Länge" .
                            "'>"
            ...
            echo "<td><input name='indexes[$x][name]' value='" .
            h($v["name"]) .
            "' autocapitalize='off' aria-labelledby='label-name'>\n",
                "<td><input type='image' class='icon' name='drop_col[$x]' src='" .
                    h(
                        preg_replace("~\\?.*~", "", ME) .
                            "?file=cross.gif&version=4.8.1"
                    ) .
...
';
    ...
    page_header(
        DB != "" ? "Datenbank ändern" : "Datenbank erstellen",
        $k,
        [],
        h(DB)
    );
...
';
    ...
    page_header("Fremdschlüssel", $k, ["table" => $a], h($a));
    if ($_POST) {
        ksort($L["source"]);
        if ($_POST["add"]) {
            $L["source"][] = "";
...
';
    ...
    $yg = array_keys(
        fields(in_array($L["table"], $rf) ? $L["table"] : reset($rf))
    );
    $pe = "this.form['change-js'].value = '1'; this.form.submit();";
    echo "<p>" .
        "Zieltabelle" .
        ": " .
        html_select("table", $rf, $L["table"], $pe) .
        "\n";
    if ($y == "pgsql") {
        echo "Schema" .
            ": " .
...
';
    ...
    page_header(
        $aa != "" ? "Ereignis ändern" . ": " . h($aa) : "Ereignis erstellen",
        $k
    );
    if (!$L && $aa != "") {
        $M = get_rows(
...
<tr><th title="datetime">Start<td><input name="STARTS" value="',
        ...
        '">
<tr><th title="datetime">Ende<td><input name="ENDS" value="',
        h($L["ENDS"]),
        '">
<tr><th>Jede<td><input type="number" name="INTERVAL_VALUE" value="',
        h($L["INTERVAL_VALUE"]),
        '" class="size"> ',
        html_select("INTERVAL_FIELD", $fd, $L["INTERVAL_FIELD"]),
        "<tr><th>Status<td>",
        html_select("STATUS", $gg, $L["STATUS"]),
        '<tr><th>Kommentar<td><input name="EVENT_COMMENT" value="',
        h($L["EVENT_COMMENT"]),
        '" data-maxlength="64">
<tr><th><td>',
        checkbox(
...
',
        $Df
            ? "Sprache" .
                ": " .
                html_select("language", $Df, $L["language"]) .
                "\n"
            : "",
        '<input type="submit" value="Speichern">
<div class="scrollable">
...
';
    ...
    if (isset($_GET["function"])) {
        echo "<tr><td>" . "Typ des Rückgabewertes";
        edit_type(
            "returns",
            $L["returns"],
            $Xa,
...
';
    ...
    if ($_POST) {
        if (
            !$k &&
            in_array($_POST["Timing"], $Tg["Timing"]) &&
            in_array($_POST["Event"], $Tg["Event"]) &&
            in_array($_POST["Type"], $Tg["Type"])
        ) {
            $ne = " ON " . table($a);
            $Lb = "DROP TRIGGER " . idf_escape($E) . ($y == "pgsql" ? $ne : "");
            $B = ME . "table=" . urlencode($a);
            if ($_POST["drop"]) {
                query_redirect($Lb, $B, "Trigger wurde entfernt.");
            } else {
                if ($E != "") {
                    queries($Lb);
                }
                ...
                if ($E != "") {
                    queries(
                        create_trigger($ne, $L + ["Type" => reset($Tg["Type"])])
                    );
                }
            }
        }
    ...
    page_header(
        $E != "" ? "Trigger ändern" . ": " . h($E) : "Trigger erstellen",
        $k,
        ["table" => $a]
    );
    echo '
...
<tr><th>Zeitpunkt<td>',
        ...
        in_array("UPDATE OF", $Tg["Event"])
            ? " <input name='Of' value='" . h($L["Of"]) . "' class='hidden'>"
            : "",
        "<tr><th>Typ<td>",
        html_select("Type", $Tg["Type"], $L["Type"]),
        '</table>
...
';
    ...
    if (
        isset($_GET["host"]) &&
        ($J = $e->query("SHOW GRANTS FOR " . q($fa) . "@" . q($_GET["host"])))
    ) {
        while ($L = $J->fetch_row()) {
            if (
        ...
        if ($_POST["drop"]) {
            ...
            if (!$k) {
                ...
                foreach ($Yd as $ge => $r) {
                    ...
                    if (isset($_GET["grant"])) {
                        $_f = array_diff(
                            array_keys(array_filter($Yd[$ge], "strlen")),
                            $r
                        );
                    } elseif ($me == $ae) {
                    ...
                    if (
                        preg_match('~^(.+)\s*(\(.*\))?$~U', $ge, $C) &&
                        (!grant("REVOKE", $_f, $C[2], " ON $C[1] FROM $ae") ||
    ...
    if ($_POST) {
        ...
        $Gc[
            (DB == "" || $Gc ? "" : idf_escape(addcslashes(DB, "%_\\"))) . ".*"
        ] = [];
    }
    echo '<form action="" method="post">
<table cellspacing="0" class="layout">
<tr><th>Server<td><input name="host" data-maxlength="60" value="',
        h($L["host"]),
        '" autocapitalize="off">
<tr><th>Benutzer<td><input name="user" data-maxlength="80" value="',
        h($L["user"]),
        '" autocapitalize="off">
<tr><th>Passwort<td><input name="pass" id="pass" value="',
        h($L["pass"]),
...
';
    if (isset($_GET["host"])) {
        echo '<input type="submit" name="drop" value="Entfernen">',
            confirm(sprintf("Drop %s?", "$fa@$_GET[host]"));
    }
    echo '<input type="hidden" name="token" value="',
        $T,
        '">
...
';
    ...
    if (support("kill")) {
        if ($_POST && !$k) {
            ...
            queries_redirect(
                ME . "processlist=",
                lang(["%d Prozess gestoppt.", "%d Prozesse gestoppt."], $pd),
                $pd || !$_POST["kill"]
            );
        }
    }
    page_header("Prozessliste", $k);
    echo '
<form action="" method="post">
<div class="scrollable">
...
';
    if (support("kill")) {
        echo $t + 1 . "/" . sprintf("%d insgesamt", max_connections()),
            "<p><input type='submit' value='" . "Anhalten" . "'>\n";
    }
    echo '<input type="hidden" name="token" value="',
        $T,
        '">
...
',
    ...
    if ($_POST && !$k) {
        ...
        if (!$b->selectEmailProcess($Z, $o)) {
            if ($_POST["save"] || $_POST["delete"]) {
                ...
                $D = sprintf("%d Artikel betroffen.", $na);
                if ($_POST["clone"] && $J && $na == 1) {
                    $ud = last_id();
                    if ($ud) {
                        $D = sprintf("Datensatz%s wurde eingefügt.", " $ud");
                    }
                }
                queries_redirect(
                    remove_from_uri(
                ...
                if (!$_POST["val"]) {
                    $k = "Ctrl+Klick zum Bearbeiten des Wertes.";
                } else {
                    $J = true;
                    $na = 0;
                    ...
                    queries_redirect(
                        remove_from_uri(),
                        sprintf("%d Artikel betroffen.", $na),
                        $J
                    );
                }
                ...
                $k = "Die Datei muss UTF-8 kodiert sein.";
            } else {
                cookie(
                    "adminer_import",
                    "output=" .
    ...
    if (is_ajax()) {
        ...
        page_header("Daten zeigen von" . ": $rg", $k);
    }
    $P = null;
    ...
    if (!$d && support("table")) {
        ...
        if (!$J) {
            ...
            if (!$M) {
                echo "<p class='message'>" . "Keine Datensätze." . "\n";
            } else {
                    ...
                    foreach ($ah as $z => $X) {
                        if (
                            ...
                            $z =
                                "MD5(" .
                                ($y != "sql" ||
                                preg_match("~^utf8~", $m[$z]["collation"])
                                    ? $z
                                    : "CONVERT($z USING " . charset($e) . ")") .
                                ")";
                            $X = md5($X);
                        }
                        $bh .=
                    ...
                    echo "<tr" .
                        ...
                        (!$s && $N
                            ...
                            : "<td>" .
                                ...
                                ($jd || information_schema(DB)
                                    ? ""
                                    : " <a href='" .
                                        h(ME . "edit=" . urlencode($a) . $bh) .
                                        "' class='edit'>" .
                                        "bearbeiten" .
                                        "</a>"));
                    foreach ($L as $z => $X) {
                                ...
                                echo ">" .
                                    ($Bg
                                        ? "<textarea name='$u' cols='30' rows='" .
                                            (substr_count($L[$z], "\n") + 1) .
                                            "'>$Lc</textarea>"
                                        : "<input name='$u' value='$Lc' size='$_d[$z]'>");
                            } else {
                                $Dd = strpos($X, "<i>…</i>");
                                echo " data-text='" .
                                    ($Dd ? 2 : ($Bg ? 1 : 0)) .
            ...
            if (!is_ajax()) {
                if ($M || $F) {
                    ...
                    if ($_GET["page"] != "last") {
                        if ($_ == "" || (count($M) < $_ && ($M || !$F))) {
                            ...
                            if ($Dc < max(1e4, 2 * ($F + 1) * $_)) {
                                $Dc = reset(
                                    slow_query(count_rows($a, $Z, $jd, $s))
                                );
                            } else {
                                $ic = false;
                            }
                    ...
                    if ($He) {
                        echo ($Dc === false ? count($M) + 1 : $Dc - $F * $_) >
                    $_
                        ? '<p><a href="' .
                ...
                if ($M || $F) {
                    ...
                    $Dc !== false
                        ? ($ic ? "" : "~ ") .
                            lang(["%d Datensatz", "%d Datensätze"], $Dc)
                        : "",
...
';
                    ...
                    if ($Bc) {
                        print_fieldset(
                            "export",
                            "Exportieren" . " <span id='selected2'></span>"
                        );
                        $Fe = $b->dumpOutput();
                        echo $Fe
                        ? html_select("output", $Fe, $ma["output"]) . " "
                        : "",
                            html_select("format", $Bc, $ma["format"]),
                            " <input type='submit' name='export' value='" .
                                "Exportieren" .
                                "'>\n",
                            "</div></fieldset>\n";
    ...
    page_header($fg ? "Status" : "Variablen");
    ...
    if ($wg && !$k && !$_POST["search"]) {
        ...
        if ($_POST["truncate"]) {
            ...
            $D = "Tabellen wurden geleert (truncate).";
        } elseif ($_POST["move"]) {
            $J = move_tables(
                (array) $_POST["tables"],
                (array) $_POST["views"],
            ...
            $D = "Tabellen wurden entfernt (drop).";
        } elseif ($y != "sql") {
            $J =
                $y == "sqlite"
                    ? queries("VACUUM")
    ...
    if ($b->homepage()) {
        if ($_GET["ns"] !== "") {
            echo "<h3 id='tables-views'>" . "Tabellen und Views" . "</h3>\n";
            $vg = tables_list();
            if (!$vg) {
                echo "<p class='message'>" . "Keine Tabellen." . "\n";
            } else {
                    ...
                    if ($sh) {
                        ...
                        foreach (
                            [
                                "Engine" => [],
                                "Collation" => [],
                                "Data_length" => ["create", "Tabelle ändern"],
                                "Index_length" => ["indexes", "Indizes ändern"],
                                "Data_free" => ["edit", "Neuer Datensatz"],
                                "Auto_increment" => [
                                    "auto_increment=1&create",
                                    "Tabelle ändern",
                                ],
                                "Rows" => ["select", "Daten auswählen"],
                            ]
                            as $z => $A
                        ) {
                            $u = " id='$z-" . h($E) . "'";
                ...
                echo "<tr><td><th>" . sprintf("%d insgesamt", count($vg)),
                    "<td>" .
                        h(
                            $y == "sql"
                                ? $e->result("SELECT @@default_storage_engine")
                                : ""
                        ),
                    ...
                    if (count($h) != 1 && $y != "sqlite") {
                        $i = isset($_POST["target"])
                            ...
                            : (support("scheme")
                                ? $_GET["ns"]
                                : DB);
                        echo "<p>" . "In andere Datenbank verschieben" . ": ",
                            $h
            ...
            "</a>\n",
                support("view")
                    ...
                    : "";
            if (support("routine")) {
                echo "<h3 id='routines'>" . "Routinen" . "</h3>\n";
                $Ef = routines();
                if ($Ef) {
                    echo "<table cellspacing='0'>\n",
                        "<thead><tr><th>" .
            ...
            if (support("event")) {
                echo "<h3 id='events'>" . "Ereignisse" . "</h3>\n";
